...
%>
<% List<QuestionPostEntity> alldoubts=(List<QuestionPostEntity>) request.getAttribute("alldoubts");
   for(QuestionPostEntity doubt : alldoubts){ 
   System.out.println(doubt.getId());
   %>
     
<div class="card">
<!-- <h6><%=doubt.getId() %> </h6> -->
  <h2 class="card-title">Title:<%=doubt.getTitle() %></h2>
  <div class="card-details">
    <span class="card-tags">Tags:<%=doubt.getTags() %></span>
    <span class="card-date">Date: <%=doubt.getCretaedAt() %> </span>
 <a href="view-solution?id=<%=doubt.getId()%>"
